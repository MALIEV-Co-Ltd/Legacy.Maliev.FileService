...
public sealed class IdempotentUploadCoordinatorTests
{
    ...
    public async Task ResponseLossRetry_ReplaysExactSignedResponseWithoutSecondExecution()
    {
        ...
        Task<UploadResultResponse> Run(byte[] bytes) => coordinator.ExecuteAsync("intranet-service", "workflow-42", "maliev.com", "orders/42", [new File("part.stl", "model/stl", bytes)], (_, _) =>
        { executions++; return Task.FromResult(Response()); }, (_, _) => Task.FromResult<UploadResultResponse?>(null), default);
    }
    ...
    public async Task SameWorkflowWithDifferentBytes_ConflictsWithoutExecution()
    {
        ...
        Assert.Equal(1, executions);
        Task<UploadResultResponse> Execute(Guid _, CancellationToken __) { executions++; return Task.FromResult(Response()); }
        static Task<UploadResultResponse?> Reconcile(Guid _, CancellationToken __) => Task.FromResult<UploadResultResponse?>(null);
    }
    ...
    public async Task SameWorkflowKey_IsIsolatedAcrossSignedPrincipals()
    {
        ...
        foreach (var principal in new[] { "intranet-service-a", "intranet-service-b" })
            await coordinator.ExecuteAsync(principal, "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])], (_, _) => { executions++; return Task.FromResult(Response()); }, (_, _) => Task.FromResult<UploadResultResponse?>(null), default);
    ...
    public async Task ConcurrentReplay_HasOneExecutorAndExplicitInProgressResult()
    {
        ...
        var first = coordinator.ExecuteAsync("intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])], (_, _) => gate.Task, (_, _) => Task.FromResult<UploadResultResponse?>(null), default);
        await store.Acquired.Task;
        await Assert.ThrowsAsync<UploadIdempotencyInProgressException>(() => coordinator.ExecuteAsync("intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])], (_, _) => throw new InvalidOperationException(), (_, _) => Task.FromResult<UploadResultResponse?>(null), default));
    ...
    public async Task LeaseLoss_FailsClosedBeforeExecutorAndRetainsUnknownState()
    {
        ...
        await Assert.ThrowsAsync<UploadOutcomeUnknownException>(() => coordinator.ExecuteAsync("intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])], (_, _) => { executed = true; return Task.FromResult(Response()); }, (_, _) => Task.FromResult<UploadResultResponse?>(null), default));
    ...
    public async Task MidExecutionLeaseLoss_CancelsExecutorAndRetainsUnknownState()
    {
        ...
        await Assert.ThrowsAsync<UploadOutcomeUnknownException>(() => coordinator.ExecuteAsync(
            ...
            async (_, token) => { await Task.Delay(Timeout.InfiniteTimeSpan, token); return Response(); },
            (_, _) => Task.FromResult<UploadResultResponse?>(null), default));
    ...
    public async Task UnknownRetry_ReconcilesWithSameDeterministicGeneration()
    {
        ...
        await Assert.ThrowsAsync<UploadOutcomeUnknownException>(() => coordinator.ExecuteAsync(
            "intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])],
            (generation, _) => { firstGeneration = generation; throw new IOException("response lost"); },
            (_, _) => Task.FromResult<UploadResultResponse?>(null), default));
        var recovered = await coordinator.ExecuteAsync(
            "intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])],
            (_, _) => throw new InvalidOperationException("must not execute twice"),
            (generation, _) => { reconciledGeneration = generation; return Task.FromResult<UploadResultResponse?>(Response()); }, default);
    ...
    public async Task CompletionCancellation_RetainsExactResponseAndRetryResolvesWithoutExecution()
    {
        ...
        Task<UploadResultResponse> Run() => coordinator.ExecuteAsync("intranet-service", "workflow-42", "maliev.com", null, [new File("part.stl", "model/stl", [1])], (_, _) => { executions++; return Task.FromResult(Response()); }, (_, _) => Task.FromResult<UploadResultResponse?>(null), default);
    }
    ...
    private sealed class MemoryStore : IUploadIdempotencyStore
    {
        ...
        public Task<UploadAcquireResult> AcquireAsync(string identity, string value, CancellationToken c)
        {
            lock (sync)
            {
                if (State is null) { fingerprint = value; reservation = Guid.NewGuid().ToString("N"); State = "pending"; Acquired.TrySetResult(); return Task.FromResult(new UploadAcquireResult(UploadAcquireState.Acquired, reservation)); }
                if (fingerprint != value) return Task.FromResult(new UploadAcquireResult(UploadAcquireState.Conflict));
                return Task.FromResult<UploadAcquireResult>(State switch { "completed" => new(UploadAcquireState.Replay, Response: response), "unknown" => new(UploadAcquireState.Unknown, reservation, response), _ => new(UploadAcquireState.InProgress) });
            }
        }
        ...
        public Task ReleaseAsync(string identity, string id, CancellationToken c) { lock (sync) { State = null; fingerprint = null; reservation = null; } return Task.CompletedTask; }
    }
    ...
    private sealed class MultiIdentityMemoryStore : IUploadIdempotencyStore
    {
        ...
        public Task<UploadAcquireResult> AcquireAsync(string identity, string fingerprint, CancellationToken c) { if (!values.TryGetValue(identity, out var value)) { var reservation = Guid.NewGuid().ToString("N"); values[identity] = (fingerprint, reservation, null); return Task.FromResult(new UploadAcquireResult(UploadAcquireState.Acquired, reservation)); } return Task.FromResult(new UploadAcquireResult(UploadAcquireState.Replay, Response: value.Response)); }
