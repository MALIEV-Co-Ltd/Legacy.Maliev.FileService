...
public sealed class IdempotentUploadCoordinator(IUploadIdempotencyStore store)
{
    ...
    public async Task<UploadResultResponse> ExecuteAsync(
        ...
        Func<Guid, CancellationToken, Task<UploadResultResponse>> execute,
        Func<Guid, CancellationToken, Task<UploadResultResponse?>> reconcile,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(workflowKey)) return await execute(Guid.NewGuid(), cancellationToken);
        ...
        try { acquired = await store.AcquireAsync(identity, fingerprint, cancellationToken); }
        ...
        if (acquired.State == UploadAcquireState.Unknown)
        {
            ...
            try { reconciled = await reconcile(generation, cancellationToken); }
        ...
        try { response = await execute(generation, execution.Token); }
